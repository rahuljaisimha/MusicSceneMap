...
- **Shortest Path Game**: A game mode where the user is given two musicians and must find the shortest path between them through the graph (e.g. "Alex Turner → Arctic Monkeys → Josh Homme (produced AM) → Them Crooked Vultures → John Paul Jones → Led Zeppelin → Jimmy Page"). Could use BFS on the in-memory graph, let the user expand nodes manually to discover the path, or race against the algorithm's shortest solution. A music equivalent of "Six Degrees of Kevin Bacon."
- **Album collaborator relationships**: Use MusicBrainz recording/release relationships to find producers, guest vocalists, featured artists, and session musicians on specific albums (e.g. Josh Homme produced/sang on Arctic Monkeys' "Humbug"). These are strong connections that often indicate real-world scene proximity and would enrich the graph significantly for the shortest path game. Data lives at the recording level in MusicBrainz — flatten all per-track credits to artist-level edges during ingestion (e.g. "Josh Homme → collaborated_with → Arctic Monkeys, via: producer, album: Humbug"). Best done with the database dump rather than live API calls.
